...
describe('location review tests', () => {
    ...
    reviewTest.concurrent(
        ...
        async ({ ctx: { db, locationId1, user1, locationId2, user2 } }) => {
            ...
            expect(await getStarSummary(db, { locationId: locationId1, userId: user1.id })).toEqual({
                avg: 4.25,
                personalRating: 3.5,
                buckets: [0, 0, 0, 1, 0, 1],
            });
            expect(await getStarSummary(db, { locationId: locationId1, userId: user2.id })).toEqual({
                avg: 4.25,
                personalRating: 5,
                buckets: [0, 0, 0, 1, 0, 1],
            });
            expect(await getStarSummary(db, { locationId: locationId2, userId: user1.id })).toEqual({
                avg: null,
                personalRating: null,
                buckets: [0, 0, 0, 0, 0, 0],
            });
            expect(await getStarSummary(db, { locationId: locationId2, userId: user2.id })).toEqual({
                avg: null,
                personalRating: null,
                buckets: [0, 0, 0, 0, 0, 0],
            });
    ...
    reviewTest.concurrent('delete rating', async ({ ctx: { db, locationId1, user1, user2 } }) => {
        await addStarReview(db, {
            locationId: locationId1,
            userId: user1.id,
            rating: 2.5,
    ...
    reviewTest.concurrent('delete tag review', async ({ ctx: { db, locationId1, user1, locationId2, user2 } }) => {
        await updateTagReview(db, {
            locationId: locationId1,
            userId: user1.id,
            tagId: 1,
