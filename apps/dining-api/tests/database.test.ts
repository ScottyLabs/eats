import { addLocationDataToDb, addTimeOverride, addWeeklyTimeOverride } from 'db/updateLocation';
...
describe('general location insertion tests', () => {
    ...
    dbTest.concurrent('properly resets state on every new dbTest', async ({ ctx: { db } }) => {
        expect(await getAllLocationsFromDB(db, DateTime.now())).toEqual([]);
    });
    dbTest.concurrent('works on insertion with times', async ({ ctx: { db } }) => {
    ...
    dbTest.concurrent('works on insertion with times (tests search window)', async ({ ctx: { db } }) => {
        const id = await addLocationDataToDb(db, {
            ...locationIn,
            today: {
                year: 2025,
    ...
    dbTest.concurrent('works on insertion with times (tests search window)', async ({ ctx: { db } }) => {
        const id = await addLocationDataToDb(db, {
            ...
            times: [parseTime('1/1/25', '5:00 am', '12:00 pm'), parseTime('2/1/25', '5:00 am', '12:00 pm')],
        });
        const dbResult = await getAllLocationsFromDB(
            db,
            parseDate('1/3/25'), // 2 days after latest time
    ...
    dbTest.concurrent('works on insertion with times (DST - start 2AM -> 3AM) (3/9/25)', async ({ ctx: { db } }) => {
        const id = await addLocationDataToDb(db, {
            ...locationIn,
    ...
    dbTest.concurrent('works on specials', async ({ ctx: { db } }) => {
        const id = await addLocationDataToDb(db, {
            ...
            times: [parseTime('1/1/25', '5:00 am', '12:00 pm'), parseTime('1/1/25', '2:00 pm', '2:00am')],
            todaysSoups: [
    ...
    dbTest.concurrent('works on specials (overriding)', async ({ ctx: { db } }) => {
        const id1 = await addLocationDataToDb(db, {
            ...
            times: [parseTime('1/1/25', '5:00 am', '12:00 pm'), parseTime('1/1/25', '2:00 pm', '2:00 am')],
            todaysSoups: [
                { title: 'soup 1', description: 'desc 1' },
                { title: 'soup 2', description: 'desc 2' },
            ],
    ...
    dbTest.concurrent('works on time overwrites', async ({ ctx: { db } }) => {
        ...
        const success1 = await addTimeOverride(db, id, '1/1/25', '2:00 AM - 3:00 PM');
        const success2 = await addTimeOverride(db, id, '1/1/25', '3:00 AM - 4:00 PM'); // second one should overwrite the first one
    ...
    dbTest.concurrent('point time overwrites take precedence over weekly time overwrites', async ({ ctx: { db } }) => {
        ...
        const success = await addTimeOverride(db, locationId, '1/1/25', '5:00 AM - 6:00 AM');
    ...
    dbTest.concurrent('adding times for the next day', async ({ ctx: { db } }) => {
        const id1 = await addLocationDataToDb(db, {
            ...locationIn,
            today: {
                year: 2025,
...
function _parseTime(time: string) {
    const [_, hour, minute, ampm] = [...(/(\d*)\s*:\s*(\d*)\s*(AM|PM)/i.exec(time) ?? [])];
    if (hour === undefined || minute === undefined || ampm === undefined) throw new Error(`Malformed time ${time}`);
    const hourNum = parseInt(hour);
    const minuteNum = parseInt(minute);
    if (isNaN(hourNum) || isNaN(minuteNum)) throw new Error(`Malformed time ${time}`);
    const minutesSinceMidnight = (hourNum % 12) * 60 + minuteNum + (ampm.toLowerCase() === 'pm' ? 12 * 60 : 0);
