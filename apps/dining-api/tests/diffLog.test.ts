...
describe('test diff checking', () => {
    ...
    test('nested', () => {
        ...
        expect(diffs).toContainEqual(expect.stringMatching(/deleted.*a\/b\/c\/d.*3/));
        expect(diffs).toContainEqual(expect.stringMatching(/inserted.*a\/b\/c\/e.*extra/));
        expect(diffs).toContainEqual(expect.stringMatching(/inserted.*a\/be.*extra/));
    });
});
