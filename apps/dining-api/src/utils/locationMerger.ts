...
export default class ScrapeResultMerger {
    majorityDict: Partial<Record<number, Partial<Record<string, { cnt: number; originalData: ILocation }>>>> = {}; // Partial type because the values don't exist initially
    addLocation(location: ILocation) {
        const majorityDictLocationData = this.majorityDict[location.conceptId] ?? {};
    ...
    getMostFrequentLocations() {
        return Object.entries(this.majorityDict).map(([conceptId, freqData]) => {
            ...
            const bestMatch = Object.values(freqData).reduce((bestMatch, curVal) => {
                ...
                return bestMatch === undefined || curVal.cnt > bestMatch.cnt ? curVal : bestMatch;
            }, undefined);
            if (bestMatch === undefined) throw new Error();
            console.log(
                `${bestMatch.originalData.name} frequencies: ${Object.values(freqData).map((val) => val?.cnt)}`,
            );
