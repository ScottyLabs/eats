...
export function getObjDiffs(prevObject: T, newObject: T, path: string = '~'): string[] {
    let diffs: string[] = [];
    if (typeof prevObject === 'object' && typeof newObject === 'object') {
        if (prevObject instanceof Array && newObject instanceof Array) {
        ...
        } else if (!(prevObject instanceof Array) && !(newObject instanceof Array)) {
            // two objects, so we just compare the values key by key
            const setOfAllKeys = new Set([...Object.keys(prevObject), ...Object.keys(newObject)]);
            for (const key of setOfAllKeys) {
                let newDiffs = getObjDiffs(prevObject[key], newObject[key], path + '/' + key);
            ...
            diffs.push(`diff in object type at ${path}! ${JSON.stringify(prevObject)} ${JSON.stringify(newObject)}`);
        }
    } else {
...
export function getDiffsBetweenLocationData(prevLocations: ILocation[], newLocations: ILocation[]) {
...
function getArrayDiffs(prevArray: any[], newArray: any[], path: string) {
    ...
    const allKeys = new Set([...Object.keys(prevFreqCnt), ...Object.keys(newFreqCnt)]);
    for (const key of allKeys) {
        if (prevFreqCnt[key] !== newFreqCnt[key]) {
            if (prevFreqCnt[key] === undefined) {
                diffs.push(
                    `inserted value at ${path} ${newFreqCnt[key]! > 1 ? newFreqCnt[key] + ' times' : ''}: ${key}`,
                );
            } else if (newFreqCnt[key] === undefined) {
                diffs.push(
                    `deleted value at ${path} ${prevFreqCnt[key] > 1 ? prevFreqCnt[key] + ' times' : ''}: ${key}`,
                );
            } else {
                diffs.push(`frequency of ${key} changed from ${prevFreqCnt[key]} to ${newFreqCnt[key]}`);
            }
        }
    }
    return diffs;
