...
export class QueryUtils {
        ...
        const reports = await this.db
            ...
            .where(
                and(
                    ...
                    locationId !== undefined ? eq(reportsTable.locationId, locationId) : undefined,
                ),
            );
    ...
    async getSpecials(todayAsSQLString: string) {
        const data = await this.db.select().from(specialsTable).where(eq(specialsTable.date, todayAsSQLString));
        return data.reduce<
            Record<
                string,
                {
    ...
    async getLocationIdToDataMap(timeSearchCutoffStr: string) {
        const locationData = await this.db
            ...
            .leftJoin(
                timesTable,
                and(eq(locationDataTable.id, timesTable.locationId), gte(timesTable.date, timeSearchCutoffStr)),
            )
            .leftJoin(externalIdToInternalIdTable, eq(externalIdToInternalIdTable.internalId, locationDataTable.id));
        ...
        >((acc, { location_data, location_times, external_id_to_internal_id }) => {
            ...
            if (external_id_to_internal_id !== null) {
                acc[location_data.id]!.conceptId = external_id_to_internal_id.externalId;
            }
            return acc;
        }, {});
    }
    ...
    async getTimeOverrides(earliestDate: string) {
        ...
        const weeklyOverrides = await this.db
            ...
            .catch((e) => {
                notifySlack(`<!channel> Failed to fetch weekly time overwrites with error ${e}`);
                return [];
            });
    ...
    async getRatingsAvgsAndCounts(): Promise<[Record<string, number>, Record<string, number>]> {
        const ratings = await this.db
            .select({
                starRating:
                    ...
                    sql`cast(${avg(starReviewTable.starRating)} as decimal(4,3))`.mapWith(Number),
        ...
        const ratingsAvgs = Object.fromEntries(ratings.map((e) => [e.locationId, e.starRating]));
        const ratingsCounts = Object.fromEntries(ratings.map((e) => [e.locationId, e.count]));
