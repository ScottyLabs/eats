...
export async function applyOverrides(db: DBType, overrides: ILocationCoordinateOverwrites) {
    const idMappingTable = await db.select().from(externalIdToInternalIdTable);
    const idMapping = idMappingTable.reduce<Record<string, string>>((acc, { internalId, externalId, type }) => {
        if (type === 'concept_id')
            return {
                ...acc,
    ...
    await db
        ...
        .onConflictDoUpdate({
            ...
            set: conflictUpdateSet(overwritesTable, ['coordinateLat', 'coordinateLng']),
        });
}
