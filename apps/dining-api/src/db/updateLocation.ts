...
export async function addLocationDataToDb(db: DBType, location: ILocation) {
    ...
    await db.transaction(async (tx) => {
        await tx.insert(locationDataTable).values(locationDbEntry).onConflictDoUpdate({
            target: locationDataTable.id,
            set: locationDbEntry,
        });

        const todayAsSQLString = `${location.today.year}-${pad(location.today.month)}-${pad(location.today.day)}`;
        // add specials
        await tx
            .delete(specialsTable)
            .where(and(eq(specialsTable.locationId, internalId), eq(specialsTable.date, todayAsSQLString)));
        const specials = [
        ...
        await tx
            .delete(timesTable)
            .where(and(eq(timesTable.locationId, internalId), and(gte(timesTable.date, todayAsSQLString))));
...
export async function addWeeklyTimeOverride(
    ...
    await db
        ...
        .values(rowToInsert)
        .onConflictDoUpdate({
            target: [weeklyTimeOverwritesTable.locationId, weeklyTimeOverwritesTable.weekday],
            set: rowToInsert,
        });
    return true;
}
