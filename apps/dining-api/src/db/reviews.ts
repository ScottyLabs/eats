...
export async function initializeTags(db: DBType, tags: string[]) {
    await db.insert(tagListTable).values(tags.map((tag, i) => ({ name: tag, sortOrder: i * 10 })));
}
...
export async function updateTagReview(
    ...
    if (voteUp === null) {
        ...
        await db
            ...
            .onConflictDoUpdate({
                target: [tagReviewTable.locationId, tagReviewTable.tagId, tagReviewTable.userId],
                set: conflictUpdateSet(tagReviewTable, ['updatedAt', 'vote', 'writtenReview']),
            });
    }
}
...
export async function addStarReview(
    db: DBType,
    { locationId, userId, rating }: { locationId: string; userId: number; rating: number },
) {
    await db
...
export async function deleteStarReview(db: DBType, { locationId, userId }: { locationId: string; userId: number }) {
    await db
        .delete(starReviewTable)
        .where(and(eq(starReviewTable.locationId, locationId), eq(starReviewTable.userId, userId)));
}
export async function getStarSummary(
    ...
    const avg = allStarReviews.length
        ? allStarReviews.reduce<number>((acc, { starRating }) => acc + starRating, 0) / allStarReviews.length
        : null;
    const buckets = [0, 0, 0, 0, 0, 0];
    for (const { starRating } of allStarReviews) {
        buckets[Math.floor(starRating)]!++;
    }
    const personalRating = allStarReviews.find((review) => review.userId === userId)?.starRating ?? null;
...
export async function getTagSummary(
    ...
    const subQuery = db
        .select({
            ...
            likes: sql<number>`cast(count(*) filter (where ${tagReviewTable.vote}) as int)`.as('likes'),
        })
        .from(tagListTable)
        .leftJoin(
            tagReviewTable,
            and(eq(tagListTable.id, tagReviewTable.tagId), eq(tagReviewTable.locationId, locationId)),
        )
...
export async function getAllTagReviewsForLocation(db: DBType, { locationId }: { locationId: string }) {
    ...
    return reviews.map(({ tag_list, tag_reviews }) => ({
        id: tag_reviews.id,
        tagId: tag_reviews.tagId,
        userId: tag_reviews.userId,
        locationId: tag_reviews.locationId,
        vote: tag_reviews.vote,
