...
export async function createUserSession(db: DBType, user: User) {
    ...
    if (user.email === undefined || user.email.match(/^.+@(?:[^@]+\.)?cmu\.edu$/i) === null) {
        notifySlack(`Skipped adding user with email ${user.email}`);
        return;
    }
    let dbUserEntry = (await db.select().from(userTable).where(eq(userTable.googleId, user.googleId)))[0];
...
async function createOrUpdateUser(
    ...
    return (
        await db
            ...
            .onConflictDoUpdate({
                target: userTable.id,
                set: conflictUpdateSet(userTable, ['updatedAt', 'email', 'firstName', 'lastName', 'pictureUrl']),
            })
            .returning()
    )[0];
}
export type DBUser = typeof userTable.$inferSelect;
export async function fetchUserSession(db: DBType, sessionId: string): Promise<DBUser | null> {
    const session = (
        await db
            .select()
    ...
    if (session === undefined || +session.sessions.expiresAt < Date.now()) return null;
