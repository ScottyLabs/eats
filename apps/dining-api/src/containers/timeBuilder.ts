 ...
 * @returns
 */
export function getAllTimeSlotsFromSchedule(schedule: Element[], currentYear: number) {
    ...
    for (const rowHTML of schedule) {
        const $ = load(rowHTML);
        try {
            const [date, timeSlotsForThatDay] = $.text().replaceAll('\n', '').replace(/\s\s+/g, ' ').split(/,(.*)/); // splits only on first comma
            ...
            if (!rowDate.isValid) {
                notifySlack(`<!channel> Cannot parse date ${rowFullDateString}, derived from ${$.text()}`);
                continue;
            }
            ...
            notifySlack(`<!channel> Failed to parse row ${$.text()} ${e} ${(e as any).stack}`);
        }
    }
...
export function parseTimeSlots(timeSlotStrings: string) {
    ...
    for (const token of timeSlotStrings.split(/[,;]/).map((slot) => slot.trim())) {
        try {
