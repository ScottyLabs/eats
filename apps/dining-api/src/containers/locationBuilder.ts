...
import { ICoordinate, IDate, ILocation, ISpecial, IFullTimeRange } from '../types';
...
export default class LocationBuilder {
    static readonly CONCEPT_BASE_LINK = 'https://apps.studentaffairs.cmu.edu/dining/conceptinfo/Concept/';
    ...
    async populateDetailedInfo() {
        ...
        this.acceptsOnlineOrders = $('div.navItems.orderOnline').toArray().length > 0;
        ...
        const { times, earliestDay } = getAllTimeSlotsFromSchedule(nextSevenDays, serverDate.year);
        this.times = times;
