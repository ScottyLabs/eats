...
export async function retrieveSpecials(htmlContent: string): Promise<Record<number, ISpecial[]>> {
    ...
    for (const card of specialCards) {
        const conceptId = parseInt($(card).find('.detailsLink').attr('onclick')?.split('/')[1] ?? '');
        if (isNaN(conceptId)) {
            console.error('Failed to parse out conceptId of card', $(card).html());
            continue;
        }
        const specialBuilder = locationSpecialMap[conceptId] || new SpecialsBuilder(); // just in case two cards have the same corresponding id... you never know
